...
sudo mkdir -p /etc/roxom-live
if [[ ! -f /etc/roxom-live/roxom-live.env ]]; then
  sudo tee /etc/roxom-live/roxom-live.env >/dev/null <<'EOF'
...
EOF
  sudo chmod 600 /etc/roxom-live/roxom-live.env
fi
...
echo "  3. Copy env values from /etc/roxom-live/roxom-live.env into Coolify."
