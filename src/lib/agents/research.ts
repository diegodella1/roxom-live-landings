...
- If the topic is a direct head-to-head or matchup written as "A vs B", verify that exact event first. At least one returned source must mention both sides together in the title, URL, or direct match/report context. Generic team pages, roster pages, or standalone schedule/results pages for only one side are not enough.
- If the topic is a matchup and the event has already happened or is happening today, prioritize a direct result, box score, match report, or official competition page. Do not infer the outcome from team-level form pages.
